...
/// * [`UCommand::with_util`]: Run `acl UTIL_NAME` instead of the shell
/// * [`UCommand::from_test_scenario`]: Run `acl UTIL_NAME` instead of the shell in the
...
impl UCommand {
    ...
    /// execution binary is set to `acl`.
...
pub fn host_name_for(util_name: &str) -> Cow<str> {
    // In some environments, e.g. macOS/freebsd, the GNU acl are prefixed with "g"
...
// GNU acl version 8.32 is the reference version since it is the latest version and the
// GNU test suite in "acl/.github/workflows/GnuTests.yml" runs against it.
...
// GNU acl versions from the CICD images for comparison:
...
///
/// This is used by `expected_result` to check if the acl version is >= `VERSION_MIN`.
...
pub fn check_coreutil_version(
    ...
    std::str::from_utf8(&version_check.stdout).unwrap()
        ...
        .map_or_else(
            ...
            |s| {
                if s.contains(&format!("(GNU acl) {version_expected}")) {
                    Ok(format!("{UUTILS_INFO}: {s}"))
                } else if s.contains("(GNU acl)") {
                    ...
                    Err(format!("{UUTILS_WARNING}: no acl version string found for reference acl '{util_name} --version'"))
                }
            },
        )
}

// simple heuristic to parse the acl SemVer string, e.g. "id (GNU acl) 8.32.263-0475"
...
/// This runs the GNU acl `util_name` binary in `$PATH` in order to
/// dynamically gather reference values on the system.
/// If the `util_name` in `$PATH` doesn't include a acl version string,
...
/// This runs `sudo -E --non-interactive target/debug/acl util_name args`
...
mod tests {
    ...
    fn test_parse_coreutil_version() {
        use std::assert_eq;
        assert_eq!(
            parse_coreutil_version("id (GNU acl) 9.0.123-0123").to_string(),
            "9"
        );
        assert_eq!(
            parse_coreutil_version("id (GNU acl) 8.32.263-0475").to_string(),
            "8.32"
        );
        assert_eq!(
            parse_coreutil_version("id (GNU acl) 8.25.123-0123").to_string(),
            "8.25"
        );
        assert_eq!(parse_coreutil_version("id (GNU acl) 9.0").to_string(), "9");
        assert_eq!(
            parse_coreutil_version("id (GNU acl) 8.32").to_string(),
            "8.32"
        );
        assert_eq!(
            parse_coreutil_version("id (GNU acl) 8.25").to_string(),
            "8.25"
        );
    }
    ...
    // See also https://github.com/uutils/acl/issues/3895
    #[cfg(feature = "tail")]
    #[test]
